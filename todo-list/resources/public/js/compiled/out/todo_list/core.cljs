...
(defn load-tasks []
  (GET "/tasks"
    {:handler #(reset! tasks %)
     :error-handler #(js/alert "Error loading tasks")}))

(defn add-task [task]
   (POST "/tasks"
     {:params task
      :handler (fn [response]
                 (let [new-task (:task response)]  ; Access complete task object
                   (reset! tasks (conj @tasks new-task))  ; Update tasks with new task
                  ;; No need for separate load-tasks call anymore
                   ))
...
(defn task-list [status]
  (let [filtered-tasks (filter #(= (:status %) status) @tasks)]
    [:div.tasks {:on-drag-over #(-> % .preventDefault)
                 :on-drop #(let [task @current-task]
                             (add-task (assoc task :status status))
      ...
      [:div.modal {:id "editModal"}
       [:div.modal-content
        ...
        [:select {:value @status :on-change #(reset! status (-> % .-target .-value))}
         ...
         [:option {:value "blank"} "Blank"]]
        [:button {:on-click #(do
                              (add-task {:id (:id @current-task) :name @name :date @date :assignee @assignee :status @status})
                               (reset! current-task nil) 
